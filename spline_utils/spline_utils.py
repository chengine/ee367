...
def get_qp_matrices(T, dT, ddT, dddT, ddddT, As, Bs, x0=None, xf=None):
    ...
    if x0 is not None:
        C_[:3, 0:n_e] = p0
        d_[:3] = torch.from_numpy(x0)
    if xf is not None:
        C_[3:, -n_e:] = pf
        d_[3:] = torch.from_numpy(xf)
...
class SplinePlanner():
    ...
    def calculate_b_spline_coeff(self, As, Bs, x0, xf):
        ...
        #Set up CVX problem
        A_prob, b_prob, C_prob, d_prob, Q_prob = get_qp_matrices(T_list, dT_list, ddT_list, dddT_list, ddddT_list, As, Bs, x0=x0)
        n_var = C_prob.shape[-1]
