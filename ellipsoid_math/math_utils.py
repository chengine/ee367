...
def generalized_eigen(A, B):
    ...
    L = L.view(-1, state_dim, state_dim).expand(batch_dim, -1, -1)
...
def ellipsoid_intersection_test(Sigma_A, Sigma_B, mu_A, mu_B, tau=1.):
...
def ellipsoid_intersection_test_helper(Sigma_A, Sigma_B, mu_A, mu_B):
    ...
    v_squared = (torch.bmm(Phi.transpose(1, 2), (mu_A - mu_B).unsqueeze(-1))) ** 2
    return lambdas, Phi, v_squared

def ellipsoid_K_function(lambdas, v_squared, tau):
    batchdim = lambdas.shape[0]
    ss = torch.linspace(0., 1., 100, device=device)[1:-1].view(1, -1, 1)
    return (1./tau**2)*torch.sum(v_squared.view(batchdim, 1, -1)*((ss*(1.-ss))/(1.+ss*(lambdas.view(batchdim, 1, -1)-1.))), dim=2)
...
def ellipsoid_intersection_test_fast(R_A, Scale_A, R_B, Scale_B, mu_A, mu_B, ss, return_max=False):

    # TODO: currently assuming R_B and mu_B are for a single ellipsoid
    batch, ndim = Scale_A.shape[0], Scale_A.shape[-1]

    # ss = torch.linspace(0., 1., 100, device=device)# [1:-1].reshape(1, -1)        # 1 x 100 

    lambda_a = Scale_A   # batch x ndim
    Lambda = (1-ss).view(1, -1, 1) / lambda_a.view(batch, 1, ndim)   # batch x 100 x ndim

    Lambda = torch.diag_embed(Lambda.reshape(-1, ndim))

    Lambda = Lambda.view(batch, ss.shape[-1], ndim, ndim)   # batch x 100 x ndim x ndim

    # R_A: batch x ndim x ndim
    V = torch.einsum('bjk,bikl,bml -> bijm', R_A, Lambda, R_A)

    for i in range(ndim):
        r_b_i = R_B[:, i]
        l_b_i = Scale_B[i]

        rV = torch.einsum('k, bijk -> bij', r_b_i, V)

        num = l_b_i*torch.einsum('bij, bik-> bijk', rV, rV)

        denom = ss + l_b_i * torch.einsum('k, bijk, j -> bi', r_b_i, V, r_b_i)

        V = V - (num / denom.view(batch, -1, 1, 1))

    delta = mu_A - mu_B.view(1, -1)   # batch x ndim

    KK = torch.einsum('bk, bijk, bj-> bi', delta, V, delta)

    if return_max:
        # Returns max KK value for each ellipsoid
        max_KK = torch.max(KK, dim=-1)

        max_KK_val = max_KK[0]  # batch
        max_KK_ind = max_KK[1]  # batch

        max_KK_ind = torch.stack([torch.arange(batch, device=device), max_KK_ind], dim=-1)

        max_V = V[max_KK_ind[:, 0], max_KK_ind[:, 1]]       # batch x ndim x ndim

        return max_KK_val, max_V

    else:
        # Returns the boolean result of collision across all ellipsoids
        return ~torch.all(torch.any(KK > 1., dim=-1)), None
...
def compute_supporting_hyperplanes_fast(R_A, Scale_A, R_B, Scale_B, mu_A, seed_pt, ss, tau=1.):

    Kj2, Qj = ellipsoid_intersection_test_fast(R_A, Scale_A, R_B, Scale_B, mu_A, seed_pt, ss, return_max=True)   # (batch), (batch x ndim x ndim)

    delta_j = seed_pt[None] - mu_A  # batch x ndim

    A = -torch.einsum('bk, bjk -> bj', delta_j, Qj)     # (batch x ndim)
    b = -torch.sqrt(Kj2) + torch.einsum('bi, bi -> b', A, mu_A)    # (batch)

    return A.cpu().numpy(), b.cpu().numpy()

def compute_polytope_fast(R_A, Scale_A, R_B, Scale_B, mu_A, seed_pt, ss, tau=1., A_bound=None, b_bound=None):
    # Find safe polytope in A <= b form
    A, b = compute_supporting_hyperplanes_fast(R_A, Scale_A, R_B, Scale_B, mu_A, seed_pt, ss, tau)
